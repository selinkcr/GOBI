...
output_file = "get_gene_only.csv"

# CSV Header
csv_columns = [
    "species", "gff_seqid", "gff_source", "gff_type", "gff_start", "gff_end",
    "gff_score", "gff_strand", "gff_phase", "gff_gene_id", "gff_attribute_ID",
    "gff_attribute_Name", "gff_attribute_Description", "sequence"
]
    ...
    with open(fasta_file, "r") as file:
        ...
        for line in file:
            if line.startswith(">"):
                ...
                seq_id = line.strip().split()[0][1:]  # Remove '>' and take first word as seq ID
                sequence = []
            else:
                sequence.append(line.strip())

        if seq_id:  # Add last sequence
            fasta_sequences[seq_id] = "".join(sequence)
...
def parse_gff(gff_file, fasta_dict, gene_id):
    """Extract gene and exon features and their sequences for a specific gene ID."""
    gene_info = []
    exon_info = []
    ...
    with open(gff_file, "r") as file:
        for line in file:
            ...
            # Match the correct gene using "Dbxref=GeneID:<gene_id>"
            if f"GeneID:{gene_id}" in dbxref and feature_type == "gene":
                sequence = extract_sequence(fasta_dict, seqid, start, end, strand)
                gene_info.append([seqid, source, feature_type, start, end, score, strand, phase, gene_id,
                                  attr_dict.get("ID", ""), attr_dict.get("Name", ""), attr_dict.get("description", ""), sequence])

            # Find exons associated with this gene (check Parent field)
            if feature_type == "exon" and "Parent" in attr_dict and f"gene-{gene_id}" in attr_dict["Parent"]:
                sequence = extract_sequence(fasta_dict, seqid, start, end, strand)
                exon_info.append([seqid, source, feature_type, start, end, score, strand, phase, gene_id,
                                  attr_dict.get("ID", ""), attr_dict.get("Name", ""), attr_dict.get("description", ""), sequence])

    return gene_info, exon_info
    ...
    with open(output_file, "w", newline="") as csvfile:
        ...
        for species, gene_ids in gene_dict.items():
            ...
            for gene_id in gene_ids:
                ...
                gene_info, exon_info = parse_gff(gff_file, fasta_dict, gene_id)
